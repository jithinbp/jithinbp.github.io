...
3. The software sweeps **PV1** from $0$ to $5\text{ V}$ in steps, recording $V_{A1}$ (collector voltage) at each step and computing $I_C = (V_{PV1} - V_{A1})/R_C$.
...
2. **PV1 limit is 5 V:** The collector sweep is limited to $V_{CE,max} \approx 3.3\text{ V}$. This is sufficient to clearly observe both the saturation knee and the active plateau for the base currents used.
